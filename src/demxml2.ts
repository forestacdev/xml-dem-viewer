...
export class GeoTiffGenerator {
	...
	private calcImageSize(): ImageSize {
		...
		const xLength = Math.round(
			Math.abs((bounds.upper_right.lon - bounds.lower_left.lon) / pixelSize.x)
		);

		const yLength = Math.round(
			Math.abs((bounds.upper_right.lat - bounds.lower_left.lat) / Math.abs(pixelSize.y))
		);
		...
		for (let i = 0; i < sortedMetaData.length; i++) {
			...
			if (metaData.mesh_code !== npArray.mesh_code) {
				throw new GeoTiffException(
					`Mesh code mismatch: ${metaData.mesh_code} !== ${npArray.mesh_code}`
				);
			}
	...
	// ArrayBufferとしてTIFFデータを生成（基本的な実装）
	public async generateTiffArrayBuffer(): Promise<ArrayBuffer> {
		const data = this.makeDataForGeoTiff();

		// この部分は実際のTIFF生成ライブラリ（geotiff.jsなど）を使用
		// ここでは概念的な実装を示します

		throw new Error(
			'TIFF generation requires additional library like geotiff.js or similar. ' +
				'Use the data from makeDataForGeoTiff() with your preferred TIFF library.'
		);
	}

	// ブラウザでダウンロード可能なBlobを作成
	public async generateTiffBlob(fileName: string = 'dem.tiff'): Promise<Blob> {
		const arrayBuffer = await this.generateTiffArrayBuffer();
		return new Blob([arrayBuffer], { type: 'image/tiff' });
	}

	// CSV形式でデータをエクスポート（デバッグ用）
	public exportAsCSV(): string {
		const data = this.makeDataForGeoTiff();
		const { demArray, geoTransform } = data;

		let csv = 'x,y,lon,lat,elevation\n';

		for (let y = 0; y < demArray.length; y++) {
			for (let x = 0; x < demArray[y].length; x++) {
				const elevation = demArray[y][x];
				if (elevation !== -9999) {
					// 有効なデータのみ出力
					const lon = geoTransform.upperLeftX + x * geoTransform.pixelSizeX;
					const lat = geoTransform.upperLeftY + y * geoTransform.pixelSizeY;
					csv += `${x},${y},${lon.toFixed(6)},${lat.toFixed(6)},${elevation}\n`;
				}
			}
		}
...
// Canvas描画用のヘルパー関数
export const renderDemToCanvas = (
	demArray: number[][],
	canvas: HTMLCanvasElement,
	colorScale: (elevation: number) => string = defaultColorScale
): void => {
